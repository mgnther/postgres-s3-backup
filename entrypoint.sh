...
PGPASSWORD=$DATABASE_PASSWORD pg_dump -c -U $DATABASE_USER -h $DATABASE_HOST -p $DATABASE_PORT -O -v $DATABASE | aws s3 cp - s3://$AWS_BUCKET/$FILE_PREFIX`date +%d-%m-%Y`.sql
