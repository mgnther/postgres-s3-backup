...
PGPASSWORD=$DATABASE_PASSWORD pg_dumpall -c -U $DATABASE_USER -h $DATABASE_HOST -p $DATABASE_PORT -l $DATABASE -O -v | aws s3 cp - s3://$AWS_BUCKET/$FILE_PREFIX`date +%d-%m-%Y`.sql
